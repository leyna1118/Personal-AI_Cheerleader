...
import { buildPrompt, CheerStyle, PlatformType } from './prompt.js';
...
export async function cheerleader(
  ...
  const term = config.platform === 'gitlab' ? 'MR' : 'PR';
  log(`Fetching ${term} diff and description...`);
  ...
  const prompt = buildPrompt({ diff, description, style: config.style, language: config.language, platform: config.platform });
